...
import GermanyList from '../PageList/GermanyList'
import { useIntersectionObserver, getApiGermany, handleTop } from '../ContextComponent/IntersectionObserverContext'
    ...
    const { Record, setPageResults, pageResults } = useIntersectionObserver(
        ([entry]) => {
            if (entry && entry.isIntersecting) {
                ...
                const apiUrl = getApiGermany(1, newPageResults);
                axios
                    .get(apiUrl)
                    .then((res) => {
